...
plot_info_1 = {"G23",51,18;"G23",19,18;"G23",-2,18;};
plot_imagesc_clim = {[-1,1]*0.02;[-1,1]*0.045;[0,1]};
...
for m_i = 1:size(mouse_name_cell,2)
    ...
    for phase = phases
        ...
        hold(axs1,"off")
        saveas(fig1,[cf,'fig5E_hor.png'])
        for ax = axs1
            ax.View = [-90,0];
            axis(ax,"vis3d")
        end
        saveas(fig1,[cf,'fig5E_sag.png'])
        delete(fig1)
    end
end
...
common_plot_functions.phase_diff_circlemap(cf,"tas_itilick",{["late","early"]},mouse_names,'fig5F_sup_fig9A',cmap_limit={[-1,1]*0.018,[-1,1]*0.007,[-1,1]*0.007});
