...
for mouse_name_i = 1:length(mouse_names)
    ...
    for ri = 1:length(r_to_plot)
        ...
        if k == nr * nc + 1 || ri == length(r_to_plot)
            k = 0;
            saveas(fig_1,[cf,'sup_fig5A_F_upper_',num2str(mouse_name_i),'.png'])
            delete([fig_1])
        end
    end
end
...
for phase_name_i = 1:length(phase_names)
    ...
    for mouse_name_i = 1:length(mouse_names)
        ...
        for ri = 1:length(r_to_plot)
            ...
            if k == nr * nc + 1 || ri == length(r_to_plot)
                k = 0;
                saveas(fig,[cf,'sup_fig5A_F_lower_',num2str(mouse_name_i),'.png'])
                delete(fig)
            end
        end
    end
...
common_plot_functions.phase_diff_circlemap(cf,"tas_rewmerged",{["rew1late","rew1early"]},mouse_names,'sup_fig5H',cmap_limit={[-1,1]*0.014,[-1,1]*0.013,[-1,1]*0.013});
